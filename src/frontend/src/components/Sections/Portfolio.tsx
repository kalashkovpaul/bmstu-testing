...
import {images, portfolioItems, SectionId} from '../../data/data';
...
const Portfolio: FC = memo(() => {
  ...
  useEffect(() => {
    fetch(`http://${apiConfig.url}:${apiConfig.port}${apiConfig.getWorks}`, {
      method: "GET",
    }).then(data => {
    ...
    .then((userdata) => {
      ...
      userdata.forEach((item: any) => {
        fetch(`http://${apiConfig.url}:${apiConfig.port}${apiConfig.getWork}`, {
          method: "PUT",
          body: JSON.stringify({
            workName: item
        }),
          ...
          console.log(work);
          worksRef.current = [...worksRef.current, work];
          setWorks(worksRef.current);
        });
      });
    })
    .catch(e => {
            ...
            return (
              <div className="pb-6" key={`${title}-${index}`}>
                <div
                  ...
                  <Image alt={title} layout="responsive" placeholder="blur" src={images[+image]} />
